...
import com.lianziyou.bot.config.redis.RedisLock;
...
public class FilterRepeatRequest {
    ...
    @Pointcut("@annotation(com.lianziyou.bot.annotate.AvoidRepeatRequest)")
    ...
    public Object interceptor(ProceedingJoinPoint pjp) throws Throwable {
        ...
        // 上锁
        final boolean success = redisLock.Rlock(lockKey, null, active.equals(CommonConst.ACTIVE) ? 0 : (lock.intervalTime() == 0 ? 30 : lock.intervalTime()),
            TimeUnit.SECONDS);
        if (!success) {
            throw new BussinessException(lock.msg());
        }
        return pjp.proceed();
    }
