package com.lianziyou.bot.service.baidu.impl;
...
public class BaiduServiceImpl implements BaiDuService {
    ...
    static String getAccessToken() {
        ...
        if (StringUtils.isEmpty(accessToken)) {
            ...
            String body = HttpUtil.createPost(BAIDU_TOKEN_API).header(Header.CONTENT_TYPE, ContentType.JSON.getValue())
                .header(Header.ACCEPT, ContentType.JSON.getValue()).form("client_id", sysConfig.getBaiduKey())
                .form("client_secret", sysConfig.getBaiduSecretKey()).form("grant_type", "client_credentials").execute().body();
    ...
    public String translateToEnglish(String prompt) {
        ...
        try {
            ResponseEntity<String> responseEntity = BeanConfig.getRestTemplate().getForEntity(url, String.class);
            if (responseEntity.getStatusCode() != HttpStatus.OK || CharSequenceUtil.isBlank(responseEntity.getBody())) {
                throw new BussinessException(responseEntity.getStatusCodeValue() + " - " + responseEntity.getBody());
            }
            JSONObject result = JSONObject.parseObject(responseEntity.getBody());
            if (responseEntity.getBody().contains("error_code")) {
                throw new BussinessException(result.getString("error_code") + " - " + result.getString("error_msg"));
            }
    ...
    public boolean textToExamine(String prompt) {
        String body = HttpUtil.createPost(BAIDU_TO_EXAMINE_API + "?access_token=" + getAccessToken() + "&text=" + URLUtil.encode(prompt))
            .header(Header.CONTENT_TYPE, "application/x-www-form-urlencoded").header(Header.ACCEPT, ContentType.JSON.getValue()).execute().body();
