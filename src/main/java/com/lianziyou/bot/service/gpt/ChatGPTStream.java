package com.lianziyou.bot.service.gpt;
...
public class ChatGPTStream {
    ...
    public void streamChatCompletion(ChatCompletion chatCompletion, EventSourceListener eventSourceListener) {
        ...
        try {
            ...
            String key = apiKey;
            Request request = new Request.Builder().url(apiHost + CommonConst.CPT_CHAT_URL)
                .post(RequestBody.create(MediaType.parse(ContentType.JSON.getValue()), requestBody)).header(Header.AUTHORIZATION.name(), "Bearer " + key)
                .build();
     ...
     * 流式输出
     */
    public void streamChatCompletion(List<Message> messages, EventSourceListener eventSourceListener, Integer type) {
        messages.forEach(m -> {
            m.setTime(null);
        });
        String model = type == 3 ? ChatCompletion.Model.GPT_3_5_TURBO_16K.getName() : ChatCompletion.Model.GPT_4.getName();
        ChatCompletion chatCompletion = ChatCompletion.builder().messages(messages).model(model).stream(true).build();
